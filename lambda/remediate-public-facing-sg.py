...
blocked_ports = [port,blocked_port1,blocked_port2,blocked_port3,blocked_port4,blocked_port5]
...
def remediate_sg(event, AccountId,region):
    ...
    try:
        SG = ec2.describe_security_groups(GroupIds=[event['detail']['resourceId']])
        ...
        for Rule in SG.get('SecurityGroups', [{}])[0].get('IpPermissions', ''):
            if 'ToPort' in Rule:
                ...
                CidrIp = Rule.get('IpRanges', [{}])[0].get("CidrIp", '')
